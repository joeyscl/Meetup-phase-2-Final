...
public class Schedule {
    ...
    //returns a set of CourseTime signifying all breaks (treating breaks as CourseTime)
    public Set<CourseTime> getIimesOfBreaks(String dayOfWeek) {
        SortedSet<Section> sections = getSectionsForDayOfWeek(dayOfWeek);
        Set<CourseTime> breakTimes = new HashSet<CourseTime>();
        if (sections.size() == 0) { //no classes on this day
            try {
                breakTimes.add(new CourseTime("06:00", "23:59")); //no classes, entire day is break
            ...
            CourseTime courseTime = section.getCourseTime();
            try {
                breakTimes.add(new CourseTime("06:00",courseTime.getStartTime() )); //free before first class
                breakTimes.add(new CourseTime(courseTime.getEndTime(), "23:59")); //free after last class
            } catch (IllegalCourseTimeException e) {
                e.printStackTrace();
            }
            ...
            String lastTime = section.getCourseTime().getEndTime();
            try {
                breakTimes.add(new CourseTime("6:00", section.getCourseTime().getStartTime()));
                while (it.hasNext()) {
                    section = it.next();
                    ...
                    if (calculateBreakTimeInMinutes(nextTime, lastTime) >= 60) {
                        ...
                        breakTimes.add(new CourseTime(lastTime, nextTime));
                    }
                    lastTime = nextTime;
                }
                breakTimes.add(new CourseTime(section.getCourseTime().getEndTime(), "23:59")); //free after last class
            } catch (IllegalCourseTimeException e) {
                e.printStackTrace();
            }
        }
    ...
    //returns true if free for 1 hour from specified time meetUpTime on specified days
    public boolean breakAtThisTime(String dayOfWeek,String meetUpTime) {
        Set<CourseTime> breakTimes = getIimesOfBreaks(dayOfWeek);

        Iterator<CourseTime> it = breakTimes.iterator();
        CourseTime currBreak = it.next();

        while (true) {
            if (calculateBreakTimeInMinutes( currBreak.getEndTime(), meetUpTime) > 60
                    && calculateBreakTimeInMinutes( meetUpTime, currBreak.getStartTime()) >= 0 ) {
                return true;
            }
            if (it.hasNext() ) {
                currBreak = it.next();
